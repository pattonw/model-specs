...
def affinities_with_foreground_to_boundaries(
    aff_channels, fg_channel=(0, 1), accumulator="max"
):
...
class UNetBase(nn.Module):
    ...
    def _apply_default(self, x):
        ...
        for out_conv in self.out_convs:
            outputs.append(self.final_activation(out_conv(x)))
...
def _update_conv_kwargs(kwargs, scale_factor):
    ...
    if isinstance(scale_factor, int) or scale_factor.count(
        scale_factor[0]
    ) == len(scale_factor):
        return kwargs
        ...
        padding = tuple(
            0 if factor == 1 else padding for factor in scale_factor
        )
        ...
        **conv_block_kwargs,
        ...
        self.blocks = nn.ModuleList(
            [
                ...
                for inc, outc, kwargs in zip(
                    features[:-1], features[1:], conv_kwargs
                )
            ]
        )
...
class Decoder(nn.Module):
        ...
        **conv_block_kwargs,
        ...
        self.samplers = nn.ModuleList(
            [
                ...
                for factor, inc, outc in zip(
                    scale_factors, features[:-1], features[1:]
                )
            ]
        )
    ...
    def _crop(self, x, shape):
        ...
        crop = tuple(
            [slice(sd, xsh - sd) for sd, xsh in zip(shape_diff, x.shape)]
        )
        return x[crop]
...
def get_norm_layer(norm, dim, channels, n_groups=32):
    ...
    if norm == "InstanceNorm":
        return (
            ...
            if dim == 2
            else nn.InstanceNorm3d(channels)
        )
    elif norm == "GroupNorm":
        return nn.GroupNorm(min(n_groups, channels), channels)
    elif norm == "BatchNorm":
        return (
            nn.BatchNorm2d(channels) if dim == 2 else nn.BatchNorm3d(channels)
        )
    else:
        raise ValueError(
...
class Upsampler2d(Upsampler):
    def __init__(
        self, scale_factor, in_channels, out_channels, mode="bilinear"
    ):
        super().__init__(
            scale_factor, in_channels, out_channels, dim=2, mode=mode
        )


class LSDUNet2d(UNetBase):
    def __init__(
        self,
        in_channels,
        out_channels,
        ...
        **conv_block_kwargs,
        ...
        out_convs = nn.ModuleList(
            [
                nn.Conv2d(features_decoder[-1], head_out_channels, 1)
                for head_out_channels in out_channels
            ]
        )
